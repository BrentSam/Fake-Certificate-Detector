    ...
    try:
        ...
        while enum.hasMoreElements():
            ...
            if el.supportsService("com.sun.star.text.Paragraph"):
                ...
                keywords = ["entry point", "dependencies", "execution guide", "glossary", "appendix"]
                matches = [kw for kw in keywords if kw in txt.lower()]
                if matches and count > 150:
                    view_cursor.gotoRange(el.getStart(), False)
                    page_num = view_cursor.getPage()
                    # Clean txt to safe ascii/utf8 for printing
                    safe_txt = txt.encode('ascii', errors='replace').decode('ascii')
                    print(f"[{count:03d}] Page {page_num} ({style}): '{safe_txt}'")
                count += 1
